...
async function extractWithLLM(chunks, opts = {}) {
  ...
  for (let i = 0; i < chunks.length; i += concurrency) {
    ...
    const results = await Promise.all(batch.map(chunk =>
      extractChunk(client, model, chunk, tool, typeList)
    ));
 ...
 */
async function extractChunk(client, model, chunk, tool, typeList) {
  const systemPrompt = `You are a knowledge graph extraction expert. Extract ALL named entities (people, events, ideas, places, works) and their relationships from the given text. Be thorough — don't miss obscure references. Entity types: ${typeList}.`;
